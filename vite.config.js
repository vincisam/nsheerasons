...
export default defineConfig({
  ...
  base: '/',
  define: {
    // Call through Vercel proxy routes (no CORS needed)
    'import.meta.env.VITE_DESIGN_BACKEND_URL': JSON.stringify(
      process.env.VITE_DESIGN_BACKEND_URL || '/api/ai/jewellery-design'
    ),
    'import.meta.env.VITE_RATES_BACKEND_URL': JSON.stringify(
      process.env.VITE_RATES_BACKEND_URL || '/api/rates'
    ),
    'import.meta.env.VITE_ASTRO_BACKEND_URL': JSON.stringify(
      process.env.VITE_ASTRO_BACKEND_URL || '/api/ai/stone-suggestion'
    ),
