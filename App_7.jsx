...
async function callDesignAI({ promptText, fileBlock }) {
  ...
  if (DESIGN_BACKEND_URL) {
    try {
      ...
      // match this site's actual domain).
      backendFailureReason = 'Could not reach the design backend — it may be down, misconfigured, or blocking requests from this domain (CORS).';
    }
  }

  // Fallback: call Anthropic directly with no key attached. This only succeeds
  // inside a Claude.ai artifact preview, where the call is specially authenticated
  // for the artifact host — on a real deployed site it will always fail (no key,
  // and Anthropic's API doesn't allow arbitrary browser-origin CORS requests). We
  // still attempt it (harmless if it fails), but if the backend already told us
  // why it failed, that's the actionable error — report that instead of masking
  // it with this fallback's generic failure.
  const userContent = [];
  ...
  try {
    ...
    throw new Error(backendFailureReason || 'Could not reach the AI design service — check your connection and try again.');
  }
  if (!response.ok) throw new Error(backendFailureReason || ('AI design request failed (HTTP ' + response.status + ') — please try again.'));
